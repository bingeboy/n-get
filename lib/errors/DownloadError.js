...
class DownloadError extends Error {
    ...
    determineSeverity(code) {
        ...
        if (criticalErrors.some(error => code.includes(error)))
            return 'critical';
        if (highErrors.some(error => code.includes(error)))
            return 'high';
        if (mediumErrors.some(error => code.includes(error)))
            return 'medium';
        return 'low';
    }
    /**
     ...
     * @param code - Error code
     * @returns Array of recovery action objects
     */
    generateRecoveryActions(code) {
        const actions = [];
        switch (code) {
            case 'NETWORK_TIMEOUT':
                actions.push({ action: 'retry_with_delay', params: { delay: 5000, maxRetries: 3 } }, { action: 'check_network_connection', params: {} }, { action: 'increase_timeout', params: { newTimeout: 60000 } });
                break;
            case 'DNS_RESOLUTION_FAILED':
                actions.push({ action: 'retry_with_delay', params: { delay: 2000, maxRetries: 2 } }, { action: 'check_dns_settings', params: {} }, { action: 'validate_url_format', params: {} });
                break;
            case 'HTTP_429':
            case 'TOO_MANY_REQUESTS':
                actions.push({ action: 'retry_with_backoff', params: { initialDelay: 10000, maxRetries: 5 } }, { action: 'reduce_concurrency', params: { newLimit: 1 } });
                break;
            case 'INSUFFICIENT_SPACE':
                actions.push({ action: 'cleanup_temp_files', params: {} }, { action: 'check_disk_space', params: {} }, { action: 'change_destination', params: {} });
                break;
            case 'RESUME_FAILED':
                actions.push({ action: 'delete_partial_file', params: {} }, { action: 'restart_download', params: { resumeEnabled: false } });
                break;
            case 'CHECKSUM_MISMATCH':
                actions.push({ action: 'delete_corrupted_file', params: {} }, { action: 'restart_download', params: { verifyIntegrity: true } });
                break;
            default:
                if (this.isRetryable) {
     ...
     * @param includeStack - Include stack trace in output
     * @param includeContext - Include enhanced context
    ...
    toJSON(includeStack = false, includeContext = true) {
        ...
        if (includeContext) {
            errorObj.context = this.context;
        }
        return { error: errorObj };
    }
    /**
     * Converts error to compact JSON format for AI agents
     * @returns Compact structured error object
     */
    toCompactJSON() {
        return {
            code: this.code,
     ...
     * @param useColors - Use ANSI color codes
    ...
    static networkError(originalError, url, details = {}) {
        ...
        return new DownloadError(code, `Network error for ${url}: ${originalError}`, { originalError, url, ...details });
    }
    /**
     * Creates an HTTP response error
     * @param statusCode - HTTP status code
     * @param statusText - HTTP status text
     * @param url - URL that returned the error
     * @param details - Additional context
     * @returns HTTP error instance
     */
    static httpError(statusCode, statusText, url, details = {}) {
        const code = `HTTP_${statusCode}`;
        return new DownloadError(code, `HTTP ${statusCode}: ${statusText}`, { statusCode, statusText, url, ...details });
    }
    /**
     ...
     * @param originalError - Original error message
     * @param details - Additional context
     * @returns File system error instance
     */
    static fileSystemError(operation, path, originalError, details = {}) {
        ...
        return new DownloadError(code, `File system error during ${operation} on ${path}: ${originalError}`, { operation, path, originalError, ...details });
    }
    /**
     ...
     */
    static checksumMismatch(url, algorithm, expected, actual) {
        return new DownloadError('CHECKSUM_MISMATCH', `Checksum mismatch for ${url}: expected ${algorithm}:${expected}, got ${algorithm}:${actual}. The downloaded file was discarded.`, { url, algorithm, expected, actual, retryable: false });
    }
    static validationError(field, value, reason, details = {}) {
        ...
        return new DownloadError(code, `Validation failed for ${field}: ${reason}`, { field, value, reason, ...details });
    }
    /**
     * Creates an SFTP-specific error
     * @param operation - SFTP operation that failed
     * @param originalError - Original SFTP error
     * @param details - Additional context
     * @returns SFTP error instance
     */
    static sftpError(operation, originalError, details = {}) {
        ...
        return new DownloadError(code, `SFTP ${operation} failed: ${originalError}`, { operation, originalError, ...details });
    }
}
exports.DownloadError = DownloadError;
exports.default = DownloadError;
