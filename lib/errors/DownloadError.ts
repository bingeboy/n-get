...
class DownloadError extends Error {
     ...
     */
    determineSeverity(code: string): Severity {
     ...
     */
    determineCategory(code: string): Category {
     ...
     */
    determineRetryability(code: string): boolean {
     ...
     */
    generateRecoveryActions(code: string): RecoveryAction[] {
     ...
     */
    buildErrorContext(details: Record<string, unknown>): ErrorContext {
        return {
     ...
     */
    toJSON(includeStack = false, includeContext = true): { error: ErrorJSONShape } {
        const errorObj: ErrorJSONShape = {
            code: this.code,
     ...
     */
    toCompactJSON(): Record<string, unknown> {
        return {
            code: this.code,
     ...
     */
    toFormattedString(useColors = true): string {
     ...
     */
    static networkError(originalError: string, url: string, details: Record<string, unknown> = {}): DownloadError {
     ...
     */
    static httpError(statusCode: number, statusText: string, url: string, details: Record<string, unknown> = {}): DownloadError {
     ...
     */
    static fileSystemError(operation: string, path: string, originalError: string, details: Record<string, unknown> = {}): DownloadError {
     ...
     */
    static sftpError(operation: string, originalError: string, details: Record<string, unknown> = {}): DownloadError {
        let code = 'SFTP_ERROR';
