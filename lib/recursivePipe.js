...
/**
 * Initialize HTTP agents with configuration values
 * @param {ConfigManager} configManager - Configuration manager instance
 */
function initializeHttpAgents(configManager) {
    const keepAliveConfig = configManager.get('http.keepAlive', {});
    const maxSockets = configManager.get('http.maxConnections', 20);
...
function initializeServices(options = {}) {
    ...
    // Initialize HTTP agents if configManager is provided
    if (configManager && !httpAgent) {
...
async function downloadFile(url, destination, index, total, enableResume = true, options = {}) {
    ...
    initializeServices({ quietMode, configManager });
