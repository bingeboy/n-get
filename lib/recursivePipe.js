...
function initializeServices(options = {}) {
    const { quietMode = false } = options;
    
    logger = new Logger({
        level: process.env.LOG_LEVEL || 'info',
        outputs: quietMode ? [] : ['console'], // No console output in quiet mode
        enableColors: !quietMode
    });
    ...
    securityService = new SecurityService({
        config: {
            security: {
                ...
                blockPrivateNetworks: false, // Allow for development
...
/**
 * Gets the appropriate HTTP agent based on URL protocol
...
function createProgressTracker(progressBar, fileSize, startByte = 0) {
    ...
    return new Transform({
        transform(chunk, encoding, callback) {
            ...
            // Update speed calculation every 1000ms or every 50 chunks for better performance
            if (now - lastUpdate > 1000 || chunkCount % 50 === 0) {
