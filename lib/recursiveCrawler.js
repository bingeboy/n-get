...
class RecursiveCrawler {
    ...
    cleanupMemory() {
        ...
        if (totalEntries > this.cleanupThreshold) {
            ...
            if (this.visited.size > this.maxVisitedEntries) {
                const recentEntries = Array.from(this.visited).slice(-this.maxVisitedEntries);
                this.visited.clear();
     ...
     */
    parseRobotsDisallows(robotsText) {
     ...
     */
    isAllowedByRobots(disallowed, url) {
        if (!disallowed || disallowed.length === 0) {
            return true;
        }
        const urlPath = new node_url_1.URL(url).pathname.toLowerCase();
                ...
                if (!this.discovered.has(discoveredUrl)) {
                    ...
                    if (urlType === 'downloadable') {
                        ...
                        if (!this.options.followExternalLinks
                            && discoveredUrlObject.hostname !== new node_url_1.URL(url).hostname) {
                            continue;
                        }
