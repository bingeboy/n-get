...
class Logger {
    ...
    constructor(config = {}) {
        this.config = {
            ...
            logDir: config.logDir || './.nget/logs',
    ...
    initializeLogging() {
        if (this.config.outputs.includes('file') || this.config.outputs.includes('json-file')) {
            try {
                if (!node_fs_1.default.existsSync(this.config.logDir)) {
                    node_fs_1.default.mkdirSync(this.config.logDir, { recursive: true });
                }
            }
            catch (error) {
                console.error('Failed to create log directory:', error.message);
                // Fall back to console only
                this.config.outputs = this.config.outputs.filter(output => !output.includes('file'));
            }
        }
    }
    /**
    ...
    writeToFile(logEntry, filename) {
        const filePath = node_path_1.default.join(this.config.logDir, filename);
        // Write CSV header for first entry
        if (this.config.format === 'csv' && !node_fs_1.default.existsSync(filePath)) {
            const headerLine = this.getCSVHeader() + '\n';
            node_fs_1.default.writeFileSync(filePath, headerLine);
        ...
        // Append to file
        node_fs_1.default.appendFileSync(filePath, logLine);
    ...
    rotateLogFile(filePath) {
        try {
            ...
            for (let i = this.config.maxFiles - 1; i > 0; i--) {
                const oldFile = node_path_1.default.join(dir, `${basename}.${i}${ext}`);
                const newFile = node_path_1.default.join(dir, `${basename}.${i + 1}${ext}`);
                if (node_fs_1.default.existsSync(oldFile)) {
                    if (i === this.config.maxFiles - 1) {
                        node_fs_1.default.unlinkSync(oldFile); // Delete oldest file
                    }
                    else {
                        node_fs_1.default.renameSync(oldFile, newFile);
                    }
                }
            }
            // Move current file to .1
            const rotatedFile = node_path_1.default.join(dir, `${basename}.1${ext}`);
            if (node_fs_1.default.existsSync(filePath)) {
                node_fs_1.default.renameSync(filePath, rotatedFile);
            }
            // Reset size tracking
