...
interface FlagEntry {
    ...
    arg?: string;
    description: string;
    group: 'general' | 'pipe' | 'recursive' | 'ssh' | 'agent' | 'webhook';
}
...
class CapabilitiesService {
    ...
    getCLIFlags(): FlagEntry[] {
        return [
            ...
            {             long: 'expect',        arg: '<algo>:<hex>', description: 'Verify the download against an expected checksum; discards the file and fails on mismatch (single URL)', group: 'agent' },
    ...
    toHelpSummary(): string {
        ...
        const groupOrder: (FlagEntry['group'])[] = ['general', 'pipe', 'recursive', 'ssh', 'agent', 'webhook'];
