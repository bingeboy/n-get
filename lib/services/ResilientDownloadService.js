...
class ResilientDownloadService {
    ...
    async downloadWithRetry(url, destination, options = {}) {
        ...
        if (this.circuitBreaker.enabled && this.isCircuitOpen(url)) {
            throw new DownloadError('CIRCUIT_BREAKER_OPEN', `Circuit breaker is open for ${url}. Service temporarily unavailable.`, {
                url,
        ...
        throw new DownloadError('DOWNLOAD_FAILED_AFTER_RETRIES', `Download failed after ${maxRetries + 1} attempts: ${lastError?.message}`, {
    ...
    recordFailure(url, error) {
        ...
        this.circuitBreaker.failures = this.circuitBreaker.failures.filter(failure => now - failure.timestamp < this.circuitBreaker.monitorWindow);
