 ...
 * @param reqUrl - The URL string to validate and normalize
 * @returns The normalized, valid URL string
 * @throws {Error} When the URL is invalid or uses an unsupported protocol
 */
function requestUri(reqUrl: string): string {
    ...
    try {
        ...
        try {
            ...
            console.log('No Protocol Provided, Defaulting to:', url.protocol);
            ...
            const msg = constructError instanceof Error ? constructError.message : String(constructError);
            throw new Error(`Invalid URL: ${reqUrl} - ${msg}`);
        }
    }
}
