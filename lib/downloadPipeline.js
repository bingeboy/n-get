...
function initializeServices(options = {}) {
    ...
    metadataService = new MetadataService({
        ...
        enableIntegrityChecks: configManager ? configManager.get('security.enableIntegrityChecks', true) : true,
        enableTimingMetrics: configManager ? configManager.get('monitoring.performanceTracking', true) : true,
    });

    // Initialize output formatter
    outputFormatter = new OutputFormatterService({
        logger,
        defaultFormat: 'text'
    });
}
...
async function downloadHttpFile(url, destination, index, total, enableResume = true, options = {}) {
    const {outputToStdout = false, stdoutMode = false, outputFilename = null, quietMode = false, configManager} = options;
    ...
    try {
        ...
        if (!outputToStdout && enableResume && supportsResume) {
            ...
            if (resumeInfo.canResume) {
                ...
                if (!quietMode && !stdoutMode && writePath !== originalPath) {
        ...
        // Create progress bar if file size is known and file is large enough (not in quiet mode or stdout mode)
        ...
        // Create progress tracker (skip for stdout mode to avoid interference)
        const progressTracker = stdoutMode ? 
            ...
            createProgressTracker(progressBar, totalSize, startByte, configManager);
        ...
        // Display completion with metrics (not in quiet mode or stdout mode)
        if (!quietMode && !stdoutMode) {
...
async function download(urls, destination, options = {}) {
    ...
    if (outputFormat !== 'text' && outputFormatter) {
        try {
            ...
            logger.error('Failed to format structured output', {
                format: outputFormat,
                error: error.message
            });
