...
const fetch = (...args) => Promise.resolve().then(() => __importStar(require('node-fetch'))).then(({ default: f }) => f(...args));
...
function initializeHttpAgents(configManager) {
    ...
    if (configManager) {
        ...
        keepAliveConfig = cm.get('http.keepAlive', {});
        maxSockets = cm.get('http.maxConnections', 20);
        ipv6Config = cm.get('http.ipv6', {});
    }
...
function createProgressTracker(progressBar, fileSize, startByte = 0, configManager = null, emitter = null, url = '') {
    ...
    return new node_stream_2.Transform({
...
async function downloadHttpFile(url, destination, index, total, enableResume = true, options = {}, session) {
    ...
    try {
        ...
        const actualBytes = writeStream.bytesWritten ?? totalSize;
