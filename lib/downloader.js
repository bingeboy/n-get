...
const checksumVerifier_1 = require("./checksumVerifier");
...
function createProgressTracker(progressBar, fileSize, startByte = 0, configManager = null, emitter = null, url = '') {
    ...
    const chunkUpdateFrequency = cm ?
        cm.get('downloads.chunkUpdateFrequency', 1000) : 1000;
    const chunkSize = cm ?
        cm.get('downloads.chunkSize', 50) : 50;
    const tracker = new node_stream_2.Transform({
...
async function downloadFile(url, destination, index, total, enableResume = true, options = {}) {
    ...
    const session = options._session
        ?? new DownloadSession_js_1.DownloadSession({ quietMode, configManager, agentId: options.agentId ?? null }).start();
    ...
    try {
        ...
        if (!validationResult.isValid) {
            ...
            throw DownloadError_1.DownloadError.validationError(primaryError.field, primaryError.field === 'url' ? url : destination, primaryError.message, { validationErrors: validationResult.errors });
        }
        ...
        throw new DownloadError_1.DownloadError('UNSUPPORTED_PROTOCOL', `Protocol '${protocol}' is not supported`, { protocol, supportedProtocols: ['http', 'https', 'sftp'] });
    }
    catch (error) {
        ...
        if (error instanceof DownloadError_1.DownloadError) {
            throw error;
        }
        throw new DownloadError_1.DownloadError('DOWNLOAD_FAILED', `Download failed: ${error.message}`, { originalError: error, url, destination });
    }
}
// Download HTTP/HTTPS file with enhanced progress tracking and resume support
async function downloadHttpFile(url, destination, index, total, enableResume = true, options = {}, session) {
    ...
    try {
        ...
        if (isResume && resumeInfo && !outputToStdout) {
            ...
            if (!response.ok) {
                throw DownloadError_1.DownloadError.httpError(response.status, response.statusText, url);
            }
            if (outputToStdout) {
        ...
        const downloadedBytes = progressTracker.getBytesDownloaded() || writeStream.bytesWritten || totalSize;
        ...
        if (expectChecksum && writePath && !outputToStdout) {
            const expectation = (0, checksumVerifier_1.parseExpectation)(expectChecksum);
            const verification = await (0, checksumVerifier_1.verifyFile)(writePath, expectation);
            if (!verification.ok) {
                ...
                await fs.promises.unlink(writePath).catch(() => { });
                throw DownloadError_1.DownloadError.checksumMismatch(url, expectation.algorithm, verification.expected, verification.actual);
            }
        }
        ...
        session.completeDownload(url, { path: outputToStdout ? 'stdout' : writePath, size: downloadedBytes, duration: durationMs, speed });
        return {
        ...
        if (!quietMode) {
            emitter.downloadError(url, error);
        }
        ...
        session.failDownload(url, error);
        // Wrap non-DownloadErrors
        if (!(error instanceof DownloadError_1.DownloadError)) {
            const errCode = error.code;
            if (errCode === 'ENOTFOUND' || errCode === 'EAI_NODATA') {
                throw DownloadError_1.DownloadError.networkError(error.message, url);
            }
                ...
                throw DownloadError_1.DownloadError.fileSystemError('write', destination || 'unknown', error.message);
            }
            else {
                throw new DownloadError_1.DownloadError('HTTP_DOWNLOAD_FAILED', `HTTP download failed: ${error.message}`, { originalError: error, url, destination });
            }
        }
        throw error;
    }
...
async function download(urls, destination, options = {}) {
    ...
    const session = new DownloadSession_js_1.DownloadSession({
        ...
        humanMode: options.humanMode ?? false,
    ...
    const downloadPromises = urls.map(async (url, index) => {
        try {
            ...
            const downloadResult = await concurrencyLimiter.execute(downloadFile, url, destination, index + 1, urls.length, enableResume, {
                ...
                expectChecksum: options.expectChecksum,
                _session: session,
            });
            ...
            if (!quietMode) {
                session.emitter.warn(`Failed to download: ${error.message}`);
            }
