...
/**
 * SFTP Manager for handling SSH/SFTP downloads
...
class SftpManager {
    ...
    parseSftpUrl(url) {
        try {
            ...
            if (urlObj.protocol !== 'sftp:') {
                throw new Error('Not an SFTP URL');
            }

            const connection = {
                host: urlObj.hostname,
                port: urlObj.port ? parseInt(urlObj.port) : this.defaultPort,
                username: urlObj.username || process.env.USER || 'anonymous',
                password: urlObj.password || null,
                remotePath: decodeURIComponent(urlObj.pathname),
     ...
     */
    createConnectionConfig(config, options = {}) {
        const sshConfig = {
            host: config.host,
            port: config.port,
        ...
        if (options.privateKey) {
            ...
            try {
                sshConfig.privateKey = fs.readFileSync(options.keyPath);
                sshConfig.passphrase = options.passphrase;
            } catch (error) {
    ...
    async getConnection(config, options = {}) {
        ...
        const sshConfig = this.createConnectionConfig(config, options);
    ...
    async downloadFile(url, destination, index, total, enableResume = true, options = {}) {
        ...
        try {
            ...
            let localPath;
            if (destination) {
                localPath = path.join(destination, config.filename);
            } else {
                localPath = path.join(process.cwd(), config.filename);
            }
            ...
            const totalSize = serverInfo.totalSize;
            let startByte = 0;
            let isResume = false;

            // Check for resume capability
            if (enableResume) {
                const resumeInfo = await resumeManager.checkPartialDownload(
                    url, localPath, totalSize, { 'last-modified': serverInfo.lastModified }
                );
    ...
    async closeAllConnections() {
        const closePromises = Array.from(this.connections.values()).map(async (sftp) => {
            try {
                await sftp.end();
            } catch (error) {
                // Ignore close errors
