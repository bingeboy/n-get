...
class ConfigManager {
    ...
    createValidationSchema() {
        return joi_1.default.object({
            ...
            ssh: joi_1.default.object({
                timeout: joi_1.default.number().min(1000).default(30000),
                algorithms: joi_1.default.object({
                    kex: joi_1.default.array().items(joi_1.default.string()).default([
                        'ecdh-sha2-nistp256',
                        'ecdh-sha2-nistp384',
                        'ecdh-sha2-nistp521',
                        'diffie-hellman-group14-sha256',
                    ]),
                    serverHostKey: joi_1.default.array().items(joi_1.default.string()).default([
                        'rsa-sha2-512',
                        'rsa-sha2-256',
                        'ssh-rsa',
                        'ecdsa-sha2-nistp256',
                    ]),
                    cipher: joi_1.default.array().items(joi_1.default.string()).default([
                        'aes128-gcm',
                        'aes256-gcm',
                        'aes128-ctr',
                        'aes256-ctr',
                    ]),
                    hmac: joi_1.default.array().items(joi_1.default.string()).default([
                        'hmac-sha2-256',
                        'hmac-sha2-512',
                        'hmac-sha1',
                    ]),
            ...
            webhooks: joi_1.default.object({
                // A string is shorthand for {url}; the object form adds per-URL
                // overrides. DownloadSession._parseConfigWebhooks accepts both.
                default: joi_1.default.array().items(joi_1.default.alternatives().try(joi_1.default.string().uri(), joi_1.default.object({
                    url: joi_1.default.string().uri().required(),
                    secret: joi_1.default.string().allow('').optional(),
                    headers: joi_1.default.object().pattern(joi_1.default.string(), joi_1.default.string()).optional(),
                    events: joi_1.default.array().items(joi_1.default.string()).optional(),
                }))).default([]),
                secret: joi_1.default.string().allow('').default(''),
                retry: joi_1.default.object({
                    maxAttempts: joi_1.default.number().min(1).default(3),
                    // EventSink indexes backoffMs by attempt number, so a array
                    // shorter than maxAttempts yields undefined delays.
                    backoffMs: joi_1.default.array().items(joi_1.default.number().min(0)).default([0, 500, 1000]),
                }).default(),
            }).default(),
        });
    }
